...
struct NovaIntelligenceAppCLI {
    static func main() throws {
        ...
        let indexURL: URL
        do {
            indexURL = try OpenWebUIBundle.indexURL()
        } catch {
            throw PreviewServerError.missingResources(underlying: error)
        }
...
private enum PreviewServerError: Error, LocalizedError {
    case missingResources(underlying: Error)
    case interpreterUnavailable

    var errorDescription: String? {
        switch self {
        case let .missingResources(underlying):
            return "Unable to access the embedded Nova Intelligence (beta) preview resources: \(underlying.localizedDescription)"
        case .interpreterUnavailable:
...
private final class PreviewServer {
    ...
    init(rootDirectory: URL, port: Int) throws {
        ...
        let scriptURL = try OpenWebUIBundle.previewServerScriptURL()
        process.arguments = [
            "python3",
            scriptURL.path,
            "--root",
    ...
    func start() throws {
        try process.run()
        print("Serving Nova Intelligence (beta) preview on http://127.0.0.1:\(port)")

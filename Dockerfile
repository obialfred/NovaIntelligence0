...
RUN find /app -type f \
    ...
    -print0 \
  | xargs -0 sed -i \
    -e 's/Open WebUI/Nova Intelligence/g' \
    -e 's/>WebUI</>Nova Intelligence</g' \
    -e 's/"WebUI"/"Nova Intelligence"/g' \
    -e 's/ WebUI / Nova Intelligence /g'

# --- Stop auto-appending the "(Open WebUI)" suffix (comment the logic; no newlines in sed) ---
RUN sed -i 's/WEBUI_NAME = os\.environ\.get("WEBUI_NAME", "Open WebUI")/WEBUI_NAME = os.environ.get("WEBUI_NAME", "Nova Intelligence")/' /app/backend/open_webui/env.py && \
...
# --- Remove literal "(Open WebUI)" suffix remnants in static assets ---
RUN find /app -type f -print0 | xargs -0 sed -i -E 's/[[:space:]]*\(Open WebUI\)//g'

# --- Default env overrides for Nova Intelligence ---
ENV WEBUI_NAME="Nova Intelligence" \
    BRAND_COLOR_PRIMARY="#3B2FF3" \
    BRAND_COLOR_ACCENT="#7C5CFF"

# --- Place custom Nova Intelligence assets / overrides here (optional build arg) ---
